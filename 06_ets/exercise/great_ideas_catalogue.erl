...
get_idea(Id) ->
  MS = ets:fun2ms(fun(Idea = {_, IdeaId, _, _, _, _})
    when IdeaId =:= Id -> Idea
                  end),
  case select(MS) of
    [Idea] -> {ok, Idea};
...
ideas_by_author(Author) ->
  MS = ets:fun2ms(fun(Idea = {_, _, _, IdeaAuthor, _, _})
    when IdeaAuthor =:= Author -> Idea
                  end),
  select(MS).


ideas_by_rating(Rating) ->
  MS = ets:fun2ms(fun(Idea = {_, _, _, _, IdeaRating, _})
...
groupBy(L) ->
  lists:foldl(
    fun({[A], List}, Acc) -> [{A, lists:sum(List)} | Acc] end
    , []
    , dict:to_list(
      lists:foldl(
...
get_authors() ->
  MS = ets:fun2ms(fun({_, _, _, IdeaAuthor, _, _}) -> [IdeaAuthor]
                  end),
  Authors = select(MS),
  lists:sort(fun({A1, C1}, {A2, C2}) -> {C1, A2} > {C2, A1}  end, groupBy(Authors)).
