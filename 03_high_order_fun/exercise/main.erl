...
get_stat(Champ) ->
  NumTeams = lists:foldl(fun(Team, Acc) -> {team, _, _} = Team, Acc + 1 end, 0, Champ),
  NumPlayers = lists:foldl(fun(Team, Acc) -> {team, _, Players} = Team, Acc + length(Players) end, 0, Champ),
  AgeSum = lists:foldl(fun(Team, Acc) -> {team, _, Players} = Team,
    Acc + lists:foldl(fun(Player, Acc2) -> {player, _, Age, _, _} = Player, Acc2 + Age end, 0, Players) end, 0, Champ),
  RatingSum = lists:foldl(fun(Team, Acc) -> {team, _, Players} = Team,
    Acc + lists:foldl(fun(Player, Acc2) -> {player, _, _, Rating, _} = Player,
      Acc2 + Rating end, 0, Players) end, 0, Champ),
  AvgAge = AgeSum / NumPlayers,
  AvgRating = RatingSum / NumPlayers,
...
filter_sick_players(Champ) ->
  Teams = lists:map(fun({team, Name, Players}) ->
    {team, Name, lists:filter(fun({player, _, _, _, Health}) -> Health >= 50 end, Players)} end, Champ),
...
make_pairs(Team1, Team2) ->
  {team, _, PlayersX} = Team1,
  {team, _, PlayersY} = Team2,
  [{NameX, NameY} || {player, NameX, _, RatingX, _} <- PlayersX, {player, NameY, _, RatingY, _} <- PlayersY, RatingX + RatingY > 600].
